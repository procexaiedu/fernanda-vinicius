...
export default async function UsuariosPage() {
  ...
  // Agregar métricas do mês por user_id em JS
  const metricsMap = new Map<string, { month_sales: number; month_revenue: number }>()
  for (const sale of (salesMonthRes.data ?? []) as { user_id: string; total: string | number }[]) {
    const prev = metricsMap.get(sale.user_id) ?? { month_sales: 0, month_revenue: 0 }
    metricsMap.set(sale.user_id, {
